...
function score()
{
	...
	for(i=0;i< rin.length;++i) {
		...
		console.log(scale_table,rin[i][0][0],rin[i][0][1],rawscore," ",rin[i][2+gender][rawscore]," ");
	}
	...
	for(i=0;i< scales.length;++i) {
		...
		for(j=0;j< scales[i][1].length;++j) {
			...
			switch(ans[q]) {
				...
				case "T":
					...
					if(tscale===undefined) {
						console.log(ci_table,scales[i][0][1],scales[i][0][2],q,"True",questions[q]);
					}
					break;
				case "F":
					// Increment the answer count
		...
		for(j=0;j< scales[i][2].length;++j) {
			...
			switch(ans[q]) {
				case "F":
					...
					++rawscore;
					if(tscale===undefined) {
						console.log(ci_table,scales[i][0][1],scales[i][0][2],q,"False",questions[q]);
					}
					break;
				case "T":
					++n;
		...
		if(tscale!==undefined) {
			...
			console.log(scale_table,scales[i][0][1],scales[i][0][2],rawscore,kscore===undefined?" ":kscore,tscore,percent.toPrecision(3));
	...
	console.log("Profile Elevation: "+pe.toPrecision(3));
...
function doc_write_all_questions()
{
	...
	if(i>370) { document.write("<\/div>"); }
            ...
            async function wait(tim){
                await new Promise(r => setTimeout(r, tim));
            }

//iterate and update the html "root2" element with the question
async function qqq(){
